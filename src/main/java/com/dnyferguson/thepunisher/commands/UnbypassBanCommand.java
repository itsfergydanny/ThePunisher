...
public class UnbypassBanCommand implements CommandExecutor {

    private ThePunisher plugin;

    public UnbypassBanCommand(ThePunisher plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 1) {
            sender.sendMessage(Chat.format("&cInvalid syntax. Use /unbypassban (username/uuid)"));
            return true;
        }
            ...
            public void run() {
                try(Connection con = plugin.getSql().getDatasource().getConnection()) {
                    ...
                    if (found) {
                        ...
                        plugin.logToDiscord(sender.getName(), "Remove Ban Bypass (/unbypassban)", "User " + sender.getName() + " has removed " + target + "'s ability to bypass bans.");
                    } else {
